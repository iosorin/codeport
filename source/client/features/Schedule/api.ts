...
export const api = {
    ...
    create: async (data: Contract['create']): Promise<ScheduleContract['CREATE']['response']> => {
        return http.post('/api/schedule', data).then((res) => res.data);
    },

    update: async (data: Contract['update']): Promise<ScheduleContract['UPDATE']['response']> => {
        // todo - сравнить с тем, что ожидает сервер
        return http.put('/api/schedule', data).then((res) => res.data);
    },

    remove: async (id: Contract['id']): Promise<ScheduleContract['REMOVE']['response']> => {
