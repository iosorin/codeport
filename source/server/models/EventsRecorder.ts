...
export class EventsRecorder {
    ...
    constructor(public saveOnServer: (date: number, time: number, roomID: string) => void) {}
    ...
    saveCandidate = (roomID: string) => {
        ...
        if (candidate) this.saveOnServer(candidate.startDate, candidate.minutes, candidate.roomID);
    };
    ...
    setEndDate = (roomID: string, endDate = Date.now()) => {
        ...
        candidate.minutes = Math.round((candidate.endDate - candidate.startDate) / 60000);
